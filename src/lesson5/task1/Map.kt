...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val a = (mapA + mapB).toMutableMap()
    for ((name, phone) in mapA) {
        if (mapB.containsKey(name) && phone != mapB[name]) a[name] = "$phone, ${mapB[name]}"
    }
    return a
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((name, cost) in stockPrices) {
        map[name] = map.getOrDefault(name, mutableListOf()) + cost
    }
    for ((key, value) in map) {
        val b = value.sum() / value.size
        res[key] = b
    }
    return res
}
 ...
 *   propagateHandshakes(
 *     mapOf(
 *       "key" to setOf("Mikhail", "Sveta"),
 *       "Sveta" to setOf("key"),
 *       "Mikhail" to setOf("Sveta")
 *     )
 *   ) -> mapOf(
 *          "key" to setOf("Mikhail", "Sveta"),
 *          "Sveta" to setOf("key", "Mikhail"),
 *          "Mikhail" to setOf("Sveta", "key")
 *        )
 */
...
fun getNamesOf(key: String, friends: Map<String, Set<String>>): Set<String> {
    val names = mutableSetOf<String>()
    val allNames = mutableSetOf(key)
    while (!allNames.isEmpty()) {
        val name = allNames.last()
        names.add(name)
 ...
 *     -> a changes to mutableMapOf() i.e. becomes empty
 */
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (a in list) res[a] = list.count { it == a }
    return res.filterValues { it >= 2 }
}
...
//    val prices = treasures.values.map { it.second }
    ...
    for (size in 1 until treasures.size) {
        table.add(MutableList(capacity + 1) { 0 })
        val a = treasures[names[size]]!!

        for (w in 0..capacity) {
            if ((a.first > w)) table[size].add(table[size - 1][w])
            else table[size].add(max(table[size - 1][w], table[size - 1][w - a.first] + a.second))
        }
    ...
    while (w > 0 && size > 0) {
//        val msize = size - 1
        val a = treasures[names[size - 1]]!!

        if (a.first <= w && table[size - 1][w] <= table[size - 1][w - (a.first)] + a.second) {
            w -= a.first
            res.add(names[size - 1])
        }
